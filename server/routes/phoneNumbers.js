...
router.post('/purchase', async (req, res) => {
  try {
    ...
    // Check balance ($1 to purchase)
    const purchaseCost = 1.00;
    if ((user.calling_balance || 0) < purchaseCost && user.name !== 'KENNYL' && user.role !== 'admin') {
    ...
    // Deduct balance
    if (user.name !== 'KENNYL' && user.role !== 'admin') {
